...
const BoardInbox: React.FC<InboxProps> = ({ messages, onMessageSelect }) => {
  ...
  return (
    <div className="max-w-lg mx-auto bg-white shadow-md rounded-lg overflow-hidden">
      <header className="px-4 py-2 bg-gray-800 text-white font-semibold">Board Inbox</header>
      
      <ul className="divide-y divide-gray-200">
        {boardMessages.map((message) => (
          <ListItem key={message.id} message={message} onMessageSelect={onMessageSelect} />
        ))}
      </ul>
    </div>
  );
