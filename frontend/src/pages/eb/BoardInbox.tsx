...
const BoardInbox= () => {
  ...
  const sortedConversations = Array.isArray(boardMessages)
    ...
    : [];
  const pendingConv=sortedConversations.filter((conversation: any) => conversation.messages[0]?.status==="PENDING");
  const approvedConv=sortedConversations.filter((conversation: any) => conversation.messages[0]?.status==="APPROVED");
  const rejectedConv=sortedConversations.filter((conversation: any) => conversation.messages[0]?.status==="REJECTED");

  if(!boardMessages){
    return (
      <Loading classes='w-full h-full'/>
    )
  }
    return (
